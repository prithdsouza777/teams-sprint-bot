...
call gcloud run deploy %SERVICE_NAME% ^
  ...
  --memory 512Mi ^
  --timeout 300 ^
  --update-env-vars "GOOGLE_PROJECT_ID=%GOOGLE_PROJECT_ID%,GEMINI_API_KEY=%GEMINI_API_KEY%,MICROSOFT_APP_ID=%MICROSOFT_APP_ID%,MICROSOFT_APP_PASSWORD=%MICROSOFT_APP_PASSWORD%,MICROSOFT_TENANT_ID=%MICROSOFT_TENANT_ID%,MONGODB_URL=%MONGODB_URL%,AWS_ACCESS_KEY_ID=%AWS_ACCESS_KEY_ID%,AWS_SECRET_ACCESS_KEY=%AWS_SECRET_ACCESS_KEY%,AWS_REGION=%AWS_REGION%,ACS_CONNECTION_STRING=%ACS_CONNECTION_STRING%,ACS_CALLBACK_URL=%ACS_CALLBACK_URL%,AZURE_COGNITIVE_SERVICES_ENDPOINT=%AZURE_COGNITIVE_SERVICES_ENDPOINT%,VOICE_STANDUP_WAIT_SECONDS=%VOICE_STANDUP_WAIT_SECONDS%,VOICE_STANDUP_SILENCE_TIMEOUT=%VOICE_STANDUP_SILENCE_TIMEOUT%,VOICE_STANDUP_MAX_SILENCE_RETRIES=%VOICE_STANDUP_MAX_SILENCE_RETRIES%"
